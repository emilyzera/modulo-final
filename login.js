...
function apagarRegistro(registroID){
    ...
    let confirma = window.confirm(`Tem certeza que deseja remover o recado de registro ID ${registroID}?`);
...
function prepararEdicao(registroID){
    ...
    let listaderec = JSON.parse(localStorage.getItem('Meusrecados'));
    let recEncontrado = listaderec.find((recado) => recado.registroID == registroID);

    inputRegistro.value = recEncontrado.registroID;
...
function atualizarRegistro(registroID){
    ...
    let listaderec = JSON.parse(localStorage.getItem('Meusrecados'));
    let indiceEncontrado = listaderec.findIndex((recado) => recado.registroID == registroID);
